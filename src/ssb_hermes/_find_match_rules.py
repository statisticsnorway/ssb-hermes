...
def _find_match_rule2(query: str, 
                      choices: list, 
                      score_cutoff: int = 75
                      ) -> tuple:
...
def _find_match_rule3(df_data_subset: pd.DataFrame,
    ...
    if _check_for_value(df_registry_subset, columns[3], kommunenr):
        df_registry_subset2 = df_registry_subset.loc[df_registry_subset[columns[3]] == kommunenr]
        item, match = _find_closest_value(df_registry_subset2, columns[1], adresse, score_cutoff=75)
        
    if item is None:
        if _check_for_value(df_registry_subset, columns[4], fylkenr):
            df_registry_subset2 = df_registry_subset.loc[df_registry_subset[columns[4]] == fylkenr]
            item, match = _find_closest_value(df_registry_subset2, columns[1], adresse, score_cutoff=75)
